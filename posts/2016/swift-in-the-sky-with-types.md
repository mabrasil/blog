Bem pessoas, este é apenas mais um repost - <strike>sim, outro</strike> - de um [post meu original do equinociOS](http://equinocios.com/swift/2016/03/28/swift-in-the-sky-with-types/)  -  que é basicamente um blog **bem massa** bolado pela galera da [CocoaHeads Brasil](http://www.cocoaheads.com.br/) onde, desde o primeiro dia do mês de Março - por isso o trocadilho com `equinócio` + `iOS` - vem trazendo posts bem legais sobre vários temas relacionados a rotina de um desenvolvedor *iOS*.

Se você me conhece um pouco, provavelmente estranhou essa mudança repentina nos temas de meus posts. Se sim - e se você se sentiu curioso pelo motivo -, saiba que esta se dá ao fato de, nos últimos meses eu ter voltado boa parte de meu foco a um novo desafio: desenvolver apps nativas para *iOS* - o que implica estudar coisas legais como *Objective-C* e *Swift*; mas não necessariamente direcionar meu estudo destas somente para a plataforma da Apple (vide o crescimento do uso de *Swift* no *back-end* de aplicações web).

Em suma, não significa que eu esteja abandanonando minhas paixões relacionadas *front-end*, *JavaScript*, *Programação Funcional* etc.: apenas que uma nova *galerinha* tá se juntando ao *conjuntinho* de *tópicozinhos* que eu gosto de estudar - e, provavelmente essas mudanças merecessem um post dedicado, mas a tal da *procrastinação* não deixou.
